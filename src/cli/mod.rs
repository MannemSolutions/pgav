...
impl Params {
    ...
    pub fn get_args() -> Params {
        ...
        args.sleeptime = generic::get_env_str(&args.sleeptime, &String::from("PGAVSLEEPTIME"), &String::from("5s"));
        args.timeout = generic::get_env_str(&args.timeout, &String::from("PGAVTIMEOUT"), &String::from("10s"));
        args.dsn = generic::get_env_str(&args.dsn, &String::from("PGAVDSN"), &String::from(""));
    ...
    pub fn sleepduration(&self) -> f32 {
        match DurationString::from_string(self.sleeptime.clone()) {
            Ok(ds) => match chrono::Duration::from_std(ds.into()) {
                Ok(duration) => {
                    let mut sleeptime = (duration.num_microseconds().unwrap() as f32)/1_000_000_f32;
                ...
                Err(_) => panic!("invalid value for max_wait: {} is not a Duration", self.sleeptime),
            }
            Err(_) => panic!("invalid value for max_wait: {} is not a Duration", self.sleeptime),
        }
    }
    pub fn timeoutduration(&self) -> f32 {
        match DurationString::from_string(self.timeout.clone()) {
            Ok(ds) => match chrono::Duration::from_std(ds.into()) {
                Ok(duration) => {
                    let timeout = (duration.num_microseconds().unwrap() as f32)/1_000_000_f32;
                    timeout
                },
                Err(_) => panic!("invalid value for max_wait: {} is not a Duration", self.sleeptime),
            }
